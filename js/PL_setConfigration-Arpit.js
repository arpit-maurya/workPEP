let sidebar = document.querySelector('.sidebar');
let button = document.querySelector('button');

sidebar.addEventListener('mouseenter', () => {
  sidebar.classList.toggle('active');
  let icon = document.querySelector('.logo_name>img');
  icon.src = `data:image/png;base64,`;
  document.querySelector('.hambergerBox').style.backgroundColor = '#008a36';
});

sidebar.addEventListener('mouseleave', () => {
  sidebar.classList.toggle('active');
  let icon = document.querySelector('.logo_name>img');
  icon.src = `data:image/png;base64,`;
  document.querySelector('.hambergerBox').style.backgroundColor = '';
  let check_priceList_li = document.querySelectorAll('.priceList-Div li');
  if (check_priceList_li) {
    check_priceList_li.forEach((e) => {
      e.remove();
    });
  }

  let check_booking_li = document.querySelectorAll('.booking-Div li');
  if (check_booking_li) {
    check_booking_li.forEach((e) => {
      e.remove();
    });
  }
});

let priceList = document.querySelector('.priceList');
let priceListStatus = false;
priceList.addEventListener('click', (e) => {
  priceListStatus ? (priceListStatus = false) : (priceListStatus = true);
  if (priceListStatus) {
    let div = document.querySelector('.priceList-Div');
    let li_1 = document.createElement('li');
    li_1.innerHTML = `<span class="links_name setPriceLi">Set price</span>`;
    div.append(li_1);
    let li_2 = document.createElement('li');
    li_2.innerHTML = `<span class="links_name configrationLi">Configuration</span>`;
    div.append(li_2);


    let li_3 = document.createElement('li');
    li_3.innerHTML = `<span class="links_name configrationLi">Addtional Services</span>`;
    div.append(li_3);

    let svg = document.querySelector('.priceList b');
    // console.log();
    svg.setAttribute('transform', 'rotate(45)');
  } else if (!priceListStatus) {
    let li = document.querySelectorAll('.priceList-Div li');
    li.forEach((e) => {
      e.remove();
    });
  }
});

let booking = document.querySelector('.booking');
let bookingStatus = false;
booking.addEventListener('click', (e) => {
  bookingStatus ? (bookingStatus = false) : (bookingStatus = true);
  if (bookingStatus) {
    let div = document.querySelector('.booking-Div');
    let li_1 = document.createElement('li');
    li_1.innerHTML = `<span class="links_name">Calendar</span>`;
    div.append(li_1);
    let li_2 = document.createElement('li');

    li_2.innerHTML = `<span class="links_name">Reservation list</span>`;
    div.append(li_2);

    let svg = document.querySelector('.booking b');

    svg.setAttribute('transform', 'rotate(45)');
  } else if (!bookingStatus) {
    let li = document.querySelectorAll('.booking-Div li');
    li.forEach((e) => {
      e.remove();
    });
  }
});

let checkBox_priceList = document.querySelectorAll('#flexSwitchCheckChecked');
console.log(checkBox_priceList);
checkBox_priceList.forEach((Element) => {
  if (Element.getAttribute('state') == 'weekend') {
    let value = false;
    Element.addEventListener('click', () => {
      value ? (value = false) : (value = true);
      let innerMianContent = document.querySelector('.inner-main-content');
      if (!value) {
        innerMianContent.innerHTML = `
            <div class="col-xl-9 col-md-12" >
                          <div class="row"> <label style="font-size: 13px;
                            display: flex;justify-content: center;
                            align-items: center;font-weight:500; color: #696a6d;">Standard</label>
                          </div>
                          <div class="row my-4">
                                <div class="input-group mb-3">
                                  <div  class="col-2  ps-4">
                                     <label style="font-size: 13px;
                            display: flex;justify-content: center;
                            align-items: center;font-weight:500; color: #696a6d;">price</label>
                                  </div>
                                  <input  type="text" value="60,000" style="background-color: white;"  class="form-control ms-2" aria-label="Text input with checkbox" readonly>
                                </div>
                          </div>
              </div> 
            `;
      } else if (value) {
        innerMianContent.innerHTML = `
            <div
                      style="
                        display: flex;
                        justify-content: center;
                        align-items: center;
                      "
                      class="row mt-3 inner-main-content"
                    >
                      <div class="col-xl-3 col-md-12">
                        <label
                          style="
                            font-size: 13px;
                            font-weight: 500;
                            color: #696a6d;
                          "
                          >Price</label
                        >
                      </div>
                      <div class="col-xl-9 col-md-12">
                        <div class="row">
                          <div class="col-md-6">
                            <label
                              style="
                                font-size: 13px;
                                display: flex;
                                justify-content: center;
                                align-items: center;
                                font-weight: 500;
                                color: #696a6d;

                              "
                              >Standard</label
                            >
                          </div>
                          <div class="col-sm-6">
                            <label
                              style="
                                font-size: 13px;
                                display: flex;
                                justify-content: center;
                                align-items: center;
                                font-weight: 500;
                                color: #696a6d;
                                
                              "
                              >Standard</label
                            >
                          </div>
                        </div>
                        <div class="row my-4">
                          <div class="col-6">
                            <input
                              type="text"
                              class="form-control"
                              placeholder="60,00"
                              aria-label="60,00"
                              readonly
                            />
                          </div>
                          <div class="col-6">
                            <input
                              type="text"
                              class="form-control"
                              placeholder="60,00"
                              aria-label="60,00"
                               readonly
                            />
                          </div>
                        </div>
                      </div>`;
      }
    });
  } else if (Element.getAttribute('state') == 'longStay') {
    let longStay = false;
    Element.addEventListener('click', () => {
      longStay ? (longStay = false) : (longStay = true);
      let longStayHiddenBox = document.querySelector('.longStayHiddenBox');
      if (longStay) {
        longStayHiddenBox.style.display = '';
        let innerMianContent = document.querySelector('.inner-main-content');
        innerMianContent.innerHTML = `
        
          <div class="row justify-content-around my-5 ">
          
            <div class="col-md-12 col-xl-2  justify-content-center" style="display:flex;
            font-size: 13px;
font-weight: 500;
color: #696a6d; justify-content:center; align-items:center;flex-direction:column">Price</div>
            
            <div class="col-md-12 col-xl-3" style=" display:flex; justify-content:center; align-items:center;flex-direction:column">
              <div class="row " style="font-size: 13px; font-weight: 500; color: #696a6d"> Standard</div>
              <div class="row">
                            <input class="my-2 " type="text" readonly class="inputStyle" placeholder="60,00"></div>
            </div>


            <div class="col-md-12 col-xl-3 longStayBox" style="display:none; justify-content:center; align-items:center;flex-direction:column">
              <div class="row shortSratLabel" style="font-size: 13px; font-weight: 500; color: #696a6d"> Short stay</div>
              <div class="row">
                            <input class="my-2" type="text" readonly class="inputStyle" placeholder="70,00"></div>
            </div>


            <div class="col-md-12 col-xl-3" style=" display:none; justify-content:center; align-items:center;flex-direction:column">
                <div class="row longStay" style="font-size: 13px; font-weight: 500; color: #696a6d"> Longer stay</div>
                <div class="row">
                            <input class="my-2" type="text" readonly class="inputStyle" placeholder="50,00">
                </div>
            </div>
           
          </div>
        `;
      } else if (!longStay) {
        longStayHiddenBox.style.display = 'none';
        let innerMianContent = document.querySelector('.inner-main-content');
        innerMianContent.innerHTML = `
                  <div style="
                        display: flex;
                        justify-content: center;
                        align-items: center;
                      " class="row mt-3 inner-main-content">
            <div class="col-xl-9 col-md-12">
                          <div class="row"> <label style="font-size: 13px;
                            display: flex;justify-content: center;
                            align-items: center;font-weight:500; color: #696a6d;">Standard</label>
                          </div>
                          <div class="row my-4">
                                <div class="input-group mb-3">
                                  <div class="col-2  ps-4">
                                     <label style="font-size: 13px;
                            display: flex;justify-content: center;
                            align-items: center;font-weight:500; color: #696a6d;">price</label>
                                  </div>
                                  <input type="text" value="60,000" style="background-color: white;" class="form-control ms-2" aria-label="Text input with checkbox" readonly="">
                                </div>
                          </div>
              </div> 
            </div>
                `;
      }
    });
  } else if (Element.getAttribute('state') == 'meals') {
    let mealsValue = false;
    Element.addEventListener('click', () => {
      mealsValue ? (mealsValue = false) : (mealsValue = true);

      let mealHiddenBox = document.querySelector('.mealHiddenBox');
      if (mealsValue) {
        if (mealHiddenBox) {
          mealHiddenBox.style.display = '';
        }
      } else {
        mealHiddenBox.style.display = 'none';
      }
    });
  } else if (Element.getAttribute('state') == 'localTax') {
    let loacalTaxValue = false;
    Element.addEventListener('click', () => {
      loacalTaxValue ? (loacalTaxValue = false) : (loacalTaxValue = true);

      let localTaxHiddenBox = document.querySelector('.localTaxHiddenBox');
      if (loacalTaxValue) {
        if (localTaxHiddenBox) {
          localTaxHiddenBox.style.display = '';
        }
      } else {
        localTaxHiddenBox.style.display = 'none';
      }
    });
  }
});


let longStayHiddenBoxInput1 = document.querySelector('.longStayHiddenBox-input1');
longStayHiddenBoxInput1.addEventListener('keyup', (e) => {
  if(!isNaN(e.currentTarget.value)){
    if (e.currentTarget.value !== 0) {
      let shortSratLabel = document.querySelector('.shortSratLabel');
      shortSratLabel.parentElement.style.display = 'flex';
      shortSratLabel.innerText = `Short stay - (from ${
        Number(e.currentTarget.value) + 1
      })`;
    } else {
      console.log(':b');
      let shortSratLabel = document.querySelector('.shortSratLabel');
      shortSratLabel.parentElement.style.display = 'none';
    }
  } else {
   
      console.log(':a');
      let shortSratLabel = document.querySelector('.shortSratLabel');
      shortSratLabel.parentElement.style.display = 'none';
    
  }
})



let longStayHiddenBoxInput2 = document.querySelector('.longStayHiddenBox-input2');
longStayHiddenBoxInput2.addEventListener('keyup', (e) => {
 if(!isNaN(e.currentTarget.value)){
    if (Number(e.currentTarget.value) !== 0) {
      let longStay = document.querySelector('.longStay');
      longStay.parentElement.style.display = 'flex';
      console.log(longStay.parentElement.style.display);
      longStay.innerText = `Long stay - (from ${Number(
        e.currentTarget.value
      )})`;
    } else {
      let longStay = document.querySelector('.longStay');
      longStay.parentElement.style.display = 'none';
    }
  }else{
     let longStay = document.querySelector('.longStay');
     longStay.parentElement.style.display = 'none';
  }
})
// let longStayHiddenBoxInput1 = document.querySelector('.longStayHiddenBox-input1');
longStayHiddenBoxInput1.addEventListener('keyup', (e) => {
 if(!isNaN(e.currentTarget.value)){
    if (Number(e.currentTarget.value) !== 0) {
      let shortSratLabel = document.querySelector('.shortSratLabel');
      shortSratLabel.parentElement.style.display = 'flex';
      console.log(shortSratLabel.parentElement.style.display);
      shortSratLabel.innerText = `Long stay - (from ${Number(
        e.currentTarget.value
      )+1})`;
    } else {
      let shortSratLabel = document.querySelector('.shortSratLabel');
      shortSratLabel.parentElement.style.display = 'none';
    }
  }else{
     let shortSratLabel = document.querySelector('.shortSratLabel');
     shortSratLabel.parentElement.style.display = 'none';
  }
})

